...
const Content = () => {
  ...
  return (
    <>
      <h3 className="mx-4 m-1 d-inline-block">All Ads in Pakistan </h3>
        ...
        {!loading && products.map((product) => (
          <div key={product.id} className="col">
            <div className="card h-auto Main-Card-Cnt">
              <img src={product.category && product.category.image} className="cart-img card-img-top" alt="..." />
              <div className="card-body">
                <h5 className="card-title">Price: ${product.price}</h5>
                <h5 className="card-title">{product.title}</h5>
              </div>
              <div className="d-flex align-items-center">
                <Link to={`/details/${product.id}`}><a className="Buy-Button btn  btn-primary m-2">See More</a></Link>
         <i className="fa fa-heart m-1 p-1" onClick={() => addtoCart(product)}></i>
              </div>
